...
class DashboardMetadataSerializer(DashboardBasicSerializer):
    ...
    def validate_quick_filter_ids(self, value: list[str] | None) -> list[str]:
        ...
        existing_ids = set(QuickFilter.objects.filter(team=team, id__in=value).values_list("id", flat=True))
        missing = [v for v in value if v not in {str(uid) for uid in existing_ids}]
        if missing:
...
class DashboardSerializer(DashboardMetadataSerializer):
    ...
    def create(self, validated_data: dict, *args: Any, **kwargs: Any) -> Dashboard:
        ...
        if existing_dashboard and existing_dashboard.quick_filter_ids:
            existing_qf_ids = set(
                QuickFilter.objects.filter(team_id=team_id, id__in=existing_dashboard.quick_filter_ids).values_list(
                    "id", flat=True
                )
            )
            validated_data["quick_filter_ids"] = [
                qf_id for qf_id in existing_dashboard.quick_filter_ids if qf_id in {str(uid) for uid in existing_qf_ids}
            ]
