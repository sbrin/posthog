...
def _instantiate_trace(template: dict, start: dt.datetime) -> list[dict]:
    """Create a trace from a template at the given start time."""
    trace_id = uuid.uuid4().hex
    span_ids = [uuid.uuid4().hex[:16] for _ in template["spans"]]
    spans: list[dict] = []
...
def generate_fixture_spans() -> list[dict]:
    ...
    """
    rng = random.Random(42)  # deterministic so sparkline is stable across requests
    now = dt.datetime.now(tz=dt.UTC)
    spans: list[dict] = []

    # Generate traces spread across the last 24 hours
    for minutes_ago in range(1440, 0, -1):
        ...
        for _ in range(traces_this_minute):
            ...
            spans.extend(_instantiate_trace(template, trace_start))
